...
export default function Toolbar({ countries, onFilter }: ToolbarProps) {
    ...
    const infoText = !count && search
        ...
        : search
            ...
            : region
                ? `REGION: ${region.toUpperCase()} - ${count} COUNTRIES.`
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="relative w-full sm:w-64 inline-block">
                    <button
                        ...
                        className={`w-full px-5 py-3 flex items-center justify-between rounded-xl shadow-md transition-colors ${bgText} ${baseBorder} ${open ? "border-indigo-500" : ""}`}
                    >
                                ...
                                {[{ label: "All Regions", val: "" }, ...REGIONS.map((r) => ({ label: r, val: r }))].map(({ label, val }) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`w-full text-left px-4 py-2 text-sm rounded-lg flex justify-between font-medium ${active ? "bg-indigo-600 text-white" : "hover:bg-gray-100 dark:hover:bg-gray-700"
                                                }`}
                                        >
